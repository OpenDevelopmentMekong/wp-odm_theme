...
function get_post_or_page_id_by_title($title_str, $post_type = 'topic')
{
		...
		foreach ($get_post as $page_topic) {
				$lang_tag = '[:'.odm_language_manager()->get_odm_language_manager()->get_current_language()().']';
				...
				if (odm_language_manager()->get_odm_language_manager()->get_current_language()() != 'en') {
								if (strpos($page_topic->post_title, '[:kh]') !== false) {
										...
										$page_title = trim(str_replace('<!--:'.odm_language_manager()->get_odm_language_manager()->get_current_language()().'-->', '', $page_title[1]));
