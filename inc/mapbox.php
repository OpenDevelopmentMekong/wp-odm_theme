<?php
add_action( 'add_meta_boxes', 'custom_parent_meta_boxes', 20 );
add_action('add_meta_boxes', 'od_mapbox_add_meta_box');
add_action('save_post', 'od_mapbox_save_postdata');

function custom_parent_meta_boxes($meta_boxes){
	remove_meta_box('mapbox', 'map', 'advanced');
}

function od_mapbox_add_meta_box() {
	// register the metabox
	add_meta_box(
		'od_mapbox', // metabox id
		__('Map setup new', 'odm'), // metabox title
		'od_mapbox_inner_custom_box', // metabox inner code
		'map', // post type
...
function od_mapbox_inner_custom_box($post) {
...
'';
			...
			<div>
				...
				<p>
					<?php _e('Baselayer copy right', 'odm'); ?>
					<textarea id="baselayer_copy_right_box" name="map_data[base_layer][copy_right]" rows="1" cols="70" style="vertical-align: top;"><?php echo $base_layer_url_copyright;?></textarea>
				</p>
			</div>
...
function od_mapbox_save_postdata($post_id) {
