...
<?php
	...
	<div class="sixteen columns<?php if (isset($extra_classes)): echo " ". $extra_classes; endif; ?>">
		<div class="post-list-item post-blog-layout-2-cols single_result_container">
			<?php if ($header_tag): ?>
	      <?php
	      $link = isset($post->dataset_link) ? $post->dataset_link : get_permalink($post->ID); ?>
			...
			<section class="content item-content section-content blog-content">
				<div class="float-left">
					<?php
					if ($show_thumbnail):
						$thumb_src = odm_get_thumbnail($post->ID, false, array( 535, 350), true);
						if (isset($thumb_src)):
							echo '<div class="blog-layout-img"><div class="blog-layout-thumb">'.$thumb_src.'</div></div>';
						else:
	...
	<div class="post-list-item post-blog-layout-2-cols single_result_container">
      ...
      $link = isset($post->dataset_link) ? $post->dataset_link : get_permalink($post->ID); ?>
		...
		<section class="content item-content section-content blog-content">
			<?php
			if ($show_thumbnail):
				$thumb_src = odm_get_thumbnail($post->ID, false, array( 535, 350));
				if (isset($thumb_src)):
					echo '<div class="blog-layout-img"><div class="blog-layout-thumb">'.$thumb_src.'</div></div>';
				else:
					echo_documents_cover($post->ID);
